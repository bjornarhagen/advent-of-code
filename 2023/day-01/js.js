const calibrationDocument = `9vxfg
...
const calibrationDocumentLines = calibrationDocument.split(/\n/gm);
const calibrationDocumentLinesDigits = calibrationDocumentLines.map((line) => {
  return line.split("").filter((char) => {
    return !isNaN(char);
  });
});

const calibrationValues = calibrationDocumentLinesDigits.map((line) => {
  return line[0].toString() + line[line.length - 1].toString();
});

const sumOfCalibrationValues = calibrationValues.reduce((currentSum, valueToAdd) => {
  return +currentSum + +valueToAdd;
});

console.log("Calibration number:", sumOfCalibrationValues);
